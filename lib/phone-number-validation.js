...
// ACCEPT-Y04: speech recognition may return a phone number as kana or
// kanji readings (「ゼロキューゼロ、…イチニサンヨン」) instead of digits.
// Map the common Japanese digit readings to digits before candidate
// extraction so a clearly dictated number is validated as heard —
// missing digits are never guessed or padded. Longest forms first so
// 「シチ」 wins over 「シ」 and 「キュウ」 over 「ク」.
const SPOKEN_DIGIT_WORDS = new Map([
    ['ゼロ', '0'], ['ぜろ', '0'], ['マル', '0'], ['まる', '0'], ['レイ', '0'], ['れい', '0'], ['零', '0'], ['〇', '0'],
    ['イチ', '1'], ['いち', '1'], ['一', '1'],
    ['ニ', '2'], ['に', '2'], ['二', '2'],
    ['サン', '3'], ['さん', '3'], ['三', '3'],
    ['ヨン', '4'], ['よん', '4'], ['シチ', '7'], ['しち', '7'], ['シ', '4'], ['し', '4'], ['ヨ', '4'], ['よ', '4'], ['四', '4'],
    ['ゴ', '5'], ['ご', '5'], ['五', '5'],
    ['ロク', '6'], ['ろく', '6'], ['六', '6'],
    ['ナナ', '7'], ['なな', '7'], ['七', '7'],
    ['ハチ', '8'], ['はち', '8'], ['八', '8'],
    ['キュウ', '9'], ['きゅう', '9'], ['キュー', '9'], ['きゅー', '9'], ['ク', '9'], ['く', '9'], ['九', '9']
]);
const SPOKEN_DIGIT_PATTERN = new RegExp(
    [...SPOKEN_DIGIT_WORDS.keys()].sort((a, b) => b.length - a.length).join('|'), 'g');

const normalizeInput = (value) => String(value ?? '').normalize('NFKC').trim();

// Convert Japanese digit readings to digits for candidate extraction.
// Group separators (、・ space) between dictated digits are pauses, not
// number boundaries — join them only when spoken readings were actually
// converted, so pure digit input keeps its legacy split. The reported
// normalizedInput stays the NFKC form; this runs only inside extraction.
const toDigitText = (normalizedValue) => {
    const converted = normalizedValue.replace(
        SPOKEN_DIGIT_PATTERN, (word) => SPOKEN_DIGIT_WORDS.get(word));
    if (converted === normalizedValue) return normalizedValue;
    return converted.replace(/(\d)[、・\s]+(?=\d)/g, '$1');
};
...
export function extractPhoneDigits(value) {
    return digitsOnly(toDigitText(normalizeInput(value)));
}
...
export function extractJapanesePhoneCandidates(value) {
    const normalizedInput = toDigitText(normalizeInput(value));
    const candidates = [];
    let buffer = '';
