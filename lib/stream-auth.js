...
export function verifyMediaStreamRequest(req, { secret = '', enabled = true, now = Date.now() } = {}) {
    ...
    try {
        ...
        // Twilio drops the query string on <Connect><Stream url> handshakes, so
        // the primary channel is the URL path segment — /media-stream/<token>.
        // The ?token= query is kept for local tests and non-Twilio clients.
