...
const getSummary = async (store) => {
    ...
    return {
        total: logs.length,
        callbackRequired: logs.filter((log) => Boolean(log.callbackRequired)).length,
        inProgress: logs.filter((log) => log.status === 'in_progress').length,
        completed: logs.filter((log) => log.status === 'completed').length
    };
};

const getStoreHealth = (store) => {
...
export async function registerAdminRoutes(fastify, {
    ...
    fastify.get('/api/admin/runtime-config', { preHandler }, async () => {
        const runtimeConfig = await getConfig(config);
        return sanitizeRuntimeConfig(runtimeConfig);
    });

    fastify.get('/api/admin/privacy/logging-policy', { preHandler }, async () => {
