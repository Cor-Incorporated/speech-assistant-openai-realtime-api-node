...
const NEGATE_PATTERN = /違います|ちがいます|違う|ちがう|間違|まちが|訂正|否定/;

const refreshCallerConfirmation = (toolState) => {
    const phone = toolState.callbackPhone;
    if (!phone?.valid || !phone.validatedAt || phone.confirmed === true) return;
    ...
    const readbackAt = afterValidation.find((t) => t.role === 'agent' || t.role === 'assistant')?.at;
    if (!readbackAt) return;
    for (const turn of afterValidation) {
        if (turn.role !== 'user' || turn.at <= readbackAt) continue;
