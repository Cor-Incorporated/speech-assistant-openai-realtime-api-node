...
import { findTransferToHumanToolCalls, shouldAllowHumanHandoff } from './handoff.js';
...
export function handleRealtimeToolCalls({
    ...
    if (handoffToolCalls.length > 0) {
        ...
        if (handoffBlockedByBusinessPolicy || !handoffConfig.enabled || handoffConfig.numbers.length === 0) {
            ...
            for (const toolCall of handoffToolCalls) {
                if (handoffConfig.enforceRoutingPolicy
                    && !shouldAllowHumanHandoff(toolState.turns || [], toolCall.destination)) {
                    outputs.push(functionCallOutputItem(toolCall.callId, {
                        ok: false,
                        reason: toolCall.destination === 'contract'
                            ? 'contract_request_not_detected'
                            : 'non_urgent_general_handoff',
                        instruction: toolCall.destination === 'contract'
                            ? '受託・開発・制作など仕事の依頼として確認できないため、電話転送は行いません。内容をコールセンターで受付してください。'
                            : '緊急性のある人間対応として確認できないため、電話転送は行いません。内容をコールセンターで受付し、必要な情報を確認して終話してください。'
                    }));
                    continue;
                }
