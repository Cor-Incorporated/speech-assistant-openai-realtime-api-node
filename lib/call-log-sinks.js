...
const assertRequired = (name, value, enabledName) => {
    if (!String(value || '').trim()) {
        throw new Error(`${name} is required when ${enabledName}=true`);
    }
};
...
export const isSmokeCallSid = (value) => String(value || '').trim().startsWith('CA_SMOKE');
...
export class CallLogSinks {
    constructor({
        ...
        sheetsRange = '',
        googleProjectId = '',
        suppressSmokeLogs = true
    } = {}) {
        this.firestoreEnabled = toBool(firestoreEnabled);
        this.sheetsEnabled = toBool(sheetsEnabled);
        this.suppressSmokeLogs = toBool(suppressSmokeLogs);

        if (this.firestoreEnabled) {
            assertRequired('CALL_LOG_FIRESTORE_DATABASE_ID', firestoreDatabaseId, 'CALL_LOG_FIRESTORE_ENABLED');
        }

        if (this.sheetsEnabled) {
            assertRequired('GOOGLE_SHEETS_SPREADSHEET_ID', spreadsheetId, 'CALL_LOG_SHEETS_ENABLED');
        }
        ...
        this.sheetsRange = String(sheetsRange || '').trim();
    ...
    async recordStarted(session) {
        ...
        if (this.shouldSkipSmokeLog(session)) return;
    ...
    async recordCompleted(record) {
        if (this.shouldSkipSmokeLog(record)) return;
    ...
    shouldSkipSmokeLog(entry) {
        if (!this.suppressSmokeLogs) return false;
        return isSmokeCallSid(entry?.callSid || entry?.id);
    }
