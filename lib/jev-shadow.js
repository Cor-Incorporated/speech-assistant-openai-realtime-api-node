...
export function createJevShadow(env = process.env, logger = console) {
    ...
    return {
         ...
         * @param {string} [callId] non-PII call identifier for correlation
         */
        observe(turns, contextRevision, legacyClassification, callId = '') {
            const startedAt = Date.now();
            ensureClassifier()
                .then((classifier) => classifier.classify(turns, contextRevision))
                .then((decision) => {
                    // Only non-PII fields are logged: call id, revision, model
                    // and question versions, latency and calibration scores.
                    logger.log(JSON.stringify({
                        ...
                        jev: decision.kind === 'known'
                            ? {
                                ...
                                intentProbabilities: decision.intentProbabilities,
                                riskScores: decision.riskScores
                            }
                            : {
                                uncertain: decision.reason,
                                // Independent risk evidence survives an
