...
const EMERGENCY_PATTERNS = [
    /(救急車|消防車|火事|火災)/,
    // Emergency numbers need digit boundaries — "1103" or an order/ID number
    // containing 110/119 is not an emergency call.
    /(?<![0-9０-９])(?:110|119)(?![0-9０-９])/,
...
// A caller reporting threats FROM a third party is a victim seeking help —
// not an aggressor to suppress. These reports need safety/fact-check
// escalation, never harassment handling.
const THIRD_PARTY_THREAT_PATTERNS = [
    /(第三者|他人|知人|元(夫|妻|恋人|交際相手|上司|同僚)|ストーカー|隣人)[^。！？\n]{0,30}(から|に)[^。！？\n]{0,30}(脅|脅迫|嫌がらせ|つきまと|攻撃|危害|恐喝)/,
    /(脅迫|恐喝|嫌がらせ|つきまとい)(され|されてい|された|している)/,
    /(命|身|殺)[^。！？\n]{0,15}(を|が)[^。！？\n]{0,15}(狙われ|脅かされ|危険)/,
    /(誰か|何者か|知らない人)[^。！？\n]{0,20}(に|から)[^。！？\n]{0,20}(脅|襲|つきまと|嫌がらせ)/
];
// A service/system outage report from a customer is existing support work —
// "弊社のシステムが止まっている" is not a sales pitch even though it contains
// 弊社 and システム.
const SERVICE_OUTAGE_PATTERNS = [
    /(システム|サービス|サイト|アプリ|画面|サーバ|回線)[^。！？\n]{0,25}(止まっている|止まった|落ちている|落ちた|使えない|動かない|動きません|繋がらない|つながらない|エラー|障害|ダウン|不具合|起動しない)/,
    /(止まっている|落ちている|使えない|動かない|繋がらない|障害|ダウン|不具合)[^。！？\n]{0,15}(ので|のですが|んです|状態)/
];
...
const SALES_BUSINESS_PATTERNS = [
    ...
    // A 弊社/当社 mention only signals sales when paired with solicitation
    // verbs — "弊社のシステムが止まっている" is an outage report, not a pitch.
    /(?:弊社|当社)[^。！？\n]{0,40}(ご提案|ご紹介|営業|セールス|ご案内|おすすめ|いかが)/,
    /(?:弊社|当社)の(サービス|システム|商品|製品|プラン|ツール)[^。！？\n]{0,30}(ご提案|ご紹介|ご利用|ご検討|興味|いかが)/,
    /(従来より|通常より|他社より)[^。！？\n]{0,20}(安|抑|割引|低)/
];
...
// Negated urgency ("急ぎではありません", "至急ではない") must not trigger the
// urgent-human handoff path. Strip explicit negations before matching.
const NEGATED_URGENCY_PATTERN = /(急ぎ|至急|緊急|今すぐ|早急)(?:では|じゃ)(?:ありません|ない|ございません)|(?:特に|別に)?急が(?:ない|なくても)/g;

const stripNegatedUrgency = (text) => String(text || '').replace(NEGATED_URGENCY_PATTERN, '');

export function shouldAutoHandoffGeneral(turns = []) {
    const recentUserText = stripNegatedUrgency(
        turns
...
export function isComplaintCall(turns = []) {
    ...
    return PAYMENT_DISPUTE_PATTERNS.some((pattern) => pattern.test(text))
        ...
        || THIRD_PARTY_THREAT_PATTERNS.some((pattern) => pattern.test(text));
}
...
export function isThirdPartyThreatCall(turns = []) {
    const text = recentUserText(turns);
    return THIRD_PARTY_THREAT_PATTERNS.some((pattern) => pattern.test(text));
}

export function isServiceOutageCall(turns = []) {
    const text = recentUserText(turns);
    return SERVICE_OUTAGE_PATTERNS.some((pattern) => pattern.test(text));
}

export function isCustomerHarassmentCall(turns = []) {
    // A victim reporting threats from someone else is not the aggressor.
    if (isThirdPartyThreatCall(turns)) return false;
    const text = recentUserText(turns);
...
export function isNonHandoffBusinessCall(turns = []) {
    ...
    // An outage report is existing-support work, not a business solicitation.
    if (isServiceOutageCall(turns)) return false;
...
export function isHandoffCallConnected(dialCallStatus, context = {}) {
    // Notification is not completion: a bridged dial only counts as human
    // handling when the recipient explicitly accepted in the whisper flow.
    // DialCallStatus=completed, a sent notification, or a stored context that
    // lacks the acknowledgment never prove a human took over.
    return String(dialCallStatus || '').toLowerCase() === 'completed'
        && (context?.whisperAccepted === true || context?.status === 'whisper_accepted');
}
...
// Pattern groups are exported read-only so the TypeScript routing layer can map
// the same rules to the intent taxonomy without duplicating the regexes.
export {
    ...
    THIRD_PARTY_THREAT_PATTERNS,
    SERVICE_OUTAGE_PATTERNS,
