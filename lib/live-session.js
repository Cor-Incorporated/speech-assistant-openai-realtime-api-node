    ...
    return {
        observeResponseEvent(delegationId, nested) {
            ...
            if (!pending.has(delegationId)) {
                pending.set(delegationId, { responseId: '', calls: [], seenCallIds: new Set() });
            }
            ...
            if (nestedType === 'response.output_item.done' && nested.item?.type === 'function_call') {
                // A duplicated/resent call_id yields at most one collected
                // call — the same output item is never executed twice.
                const callId = nested.item.call_id || '';
                if (callId && !entry.seenCallIds.has(callId)) {
                    entry.seenCallIds.add(callId);
                    entry.calls.push({
                        type: 'function_call',
                        call_id: callId,
...
export function toRealtimeDoneEvent(functionCalls, status = 'completed') {
    return {
        type: 'response.done',
        response: { output: functionCalls, status }
    };
}

// handleRealtimeToolCalls emits Realtime `conversation.item.create` envelopes;
// the knowledge runtime already returns a bare `function_call_output` item.
// Live expects the bare item through `response.item.create` — both shapes are
// normalized here. Returning null for a bare item silently drops the tool
// result, which leaves the provider-side call pending and every subsequent
// response.create rejected ("Submit the pending function call outputs").
export function toLiveToolResultItem(outputItem) {
    if (outputItem?.type === 'conversation.item.create' && outputItem.item) return outputItem.item;
    if (outputItem?.type === 'function_call_output' && outputItem.call_id) return outputItem;
    return null;
}
...
// Verified against the real API (probe 2026-09-19): the event requires
// `delegation_id` (null = session scope) and a `content` field — an
// `instructions` field is rejected with missing_required_parameter.
export function liveInstructionsAppend(content, eventId, delegationId = null) {
    return { type: 'session.instructions.append', event_id: eventId, delegation_id: delegationId, content };
}
