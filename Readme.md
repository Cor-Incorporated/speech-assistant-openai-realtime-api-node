...
管理UIのRealtimeモデル選択はFirestore `runtimeSettings/admin` に保存され、次回以降の新しい通話から `gpt-realtime-2.1-mini`（既定） / `gpt-realtime-2.1` / `gpt-realtime-2`（ロールバック） / `gpt-realtime-1.5` の選択が反映されます。進行中の通話には反映しません。音声は `coral` を既定にします。
