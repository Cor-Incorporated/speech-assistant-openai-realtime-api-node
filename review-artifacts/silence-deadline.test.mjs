...
describe('media-stream integration (real handler, mocked provider)', () => {
    ...
    it('I01 tool completion without audible progress must not extend silence beyond 25 seconds',async()=>{
        const count=provider.connections.length,token=createStreamToken({callSid:CALL_SID,secret:AUTH_TOKEN});
        const socket=await connectClient('/'+encodeURIComponent(token));let closedAt=null,start;
        ...
        try{
            ...
            start=Date.now();provider.injectDelegation(conn,'lookup_company_knowledge','{"query":"代表者"}');
            await waitFor(()=>conn.messages.some(e=>e.type==='response.create'));
            await sleep(11500);
            provider.injectDelegation(conn,'finish_reception','{"reason":"synthetic reply complete","callback_required":false}');
            await sleep(Math.max(0,26000-(Date.now()-start)));
            const openAt26=socket.readyState===WebSocket.OPEN;
            await waitFor(()=>socket.readyState!==WebSocket.OPEN,9000);
            console.log(JSON.stringify({case:'I01',openAt26,closeAfterMs:closedAt?closedAt-start:null,nudges:conn.messages.filter(e=>e.type==='session.instructions.append').length,toolResults:conn.toolOutputs.length,serverErrors:serverLog.split('\n').filter(x=>x.includes('failed'))}));
            assert.equal(openAt26,false,'New tool result reset the silence deadline without any audible progress');
