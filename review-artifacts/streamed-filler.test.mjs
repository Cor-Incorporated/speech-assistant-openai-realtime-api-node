...
describe('media-stream integration (real handler, mocked provider)', () => {
    ...
    it('E01 the exact D02 phrase split across transcript deltas still needs recovery',async()=>{
        ...
        try{
            ...
            const chunks=['ただいま','確認して','おります。','少々','お待ち','ください。'];
            for(const delta of chunks){conn.socket.send(JSON.stringify({type:'session.output_transcript.delta',delta}));await sleep(80);}
            await sleep(26000);
            console.log(JSON.stringify({case:'E01',transcript:chunks.join(''),wsOpen:socket.readyState===WebSocket.OPEN,watchdogNudges:conn.messages.filter(e=>e.type==='session.instructions.append').length,toolResults:conn.toolOutputs.length}));
            assert.notEqual(socket.readyState,WebSocket.OPEN,'Streaming prefixes of the same waiting phrase disabled watchdog');
        }finally{socket.close();await sleep(300);}
    });
    it('E02 D01 filler split around audio still needs recovery',async()=>{
        const count=provider.connections.length,token=createStreamToken({callSid:CALL_SID,secret:AUTH_TOKEN});
        const socket=await connectClient('/'+encodeURIComponent(token));
        try{
            sendStartFrame(socket);
            ...
            provider.injectDelegation(conn,'lookup_company_knowledge','{"query":"代表者"}');
            await waitFor(()=>conn.messages.some(e=>e.type==='response.create'));await sleep(100);
            conn.socket.send(JSON.stringify({type:'session.output_transcript.delta',delta:'少々'}));
            ...
            for(const delta of ['お','待ち','ください']){conn.socket.send(JSON.stringify({type:'session.output_transcript.delta',delta}));await sleep(80);}
            await sleep(26000);
            console.log(JSON.stringify({case:'E02',transcript:'少々お待ちください',audioPackets:8,wsOpen:socket.readyState===WebSocket.OPEN,watchdogNudges:conn.messages.filter(e=>e.type==='session.instructions.append').length,toolResults:conn.toolOutputs.length}));
            assert.notEqual(socket.readyState,WebSocket.OPEN,'A partial word after filler audio disabled watchdog');
        }finally{socket.close();await sleep(300);}
    });
