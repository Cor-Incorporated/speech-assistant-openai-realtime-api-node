...
it('B05 empty transcript delta must not cancel a stalled-tool watchdog',async()=>{
    ...
    it('C05 Responses watchdog instruction uses session scope rather than a client delegation id',async()=>{
        const count=provider.connections.length,token=createStreamToken({callSid:CALL_SID,secret:AUTH_TOKEN});
        const socket=await connectClient('/'+encodeURIComponent(token));
        try{
            sendStartFrame(socket);
            ...
            const instruction=await waitFor(()=>conn.messages.find(e=>e.type==='session.instructions.append'),12000);
            assert.equal(instruction.delegation_id,null,'Responses delegation id is not a valid client delegation id');
        }finally{socket.close();await sleep(300);}
    });
