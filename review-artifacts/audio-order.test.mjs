...
describe('media-stream integration (real handler, mocked provider)', () => {
    ...
    for(const [id,audioFirst] of [['G01-transcript-first',false],['G02-audio-first',true]]) it(id+' waiting audio/transcript order invariance',async()=>{
        ...
        try{
            ...
            const text=()=>conn.socket.send(JSON.stringify({type:'session.output_transcript.delta',delta:'少々お待ちくださいませ'}));
            if(!audioFirst)text();
            for(let i=0;i<40;i++){conn.socket.send(JSON.stringify({type:'session.output_audio.delta',delta:Buffer.from(Array.from({length:160},(_,n)=>n%2?0xd5:0x55)).toString('base64')}));await sleep(20);}
            if(audioFirst)text();
            await sleep(26000);
            console.log(JSON.stringify({case:id,audioPackets:40,audioBytes:6400,transcript:'少々お待ちくださいませ',wsOpen:socket.readyState===WebSocket.OPEN,watchdogNudges:conn.messages.filter(e=>e.type==='session.instructions.append').length,toolResults:conn.toolOutputs.length}));
            assert.notEqual(socket.readyState,WebSocket.OPEN,'Audio before filler transcript disarmed watchdog');
