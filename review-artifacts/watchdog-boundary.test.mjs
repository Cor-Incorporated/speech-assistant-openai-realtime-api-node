...
describe('media-stream integration (real handler, mocked provider)', () => {
    ...
    it('D01 filler audio across eight packets must not count as a completed knowledge answer',async()=>{
        ...
        try{
            ...
            for(let i=0;i<8;i++){
                conn.socket.send(JSON.stringify({type:'session.output_audio.delta',delta:Buffer.from(Array.from({length:160},(_,n)=>n%2?0xd5:0x55)).toString('base64')}));
                await sleep(20);
            }
            await sleep(26000);
            console.log(JSON.stringify({case:'D01',wsOpen:socket.readyState===WebSocket.OPEN,watchdogNudges:conn.messages.filter(e=>e.type==='session.instructions.append').length,toolResults:conn.toolOutputs.length}));
            assert.notEqual(socket.readyState,WebSocket.OPEN,'Eight filler packets disarmed watchdog; no answer and no recovery for 26 seconds');
        }finally{socket.close();await sleep(300);}
    });
    it('D02 longer waiting phrase must not count as a completed knowledge answer',async()=>{
        const count=provider.connections.length,token=createStreamToken({callSid:CALL_SID,secret:AUTH_TOKEN});
        const socket=await connectClient('/'+encodeURIComponent(token));
        try{
            sendStartFrame(socket);
            ...
            await waitFor(()=>conn.messages.some(e=>e.type==='response.create'));
            await sleep(100);
            conn.socket.send(JSON.stringify({type:'session.output_transcript.delta',delta:'ただいま確認しております。少々お待ちください。'}));
            await sleep(26000);
            console.log(JSON.stringify({case:'D02',wsOpen:socket.readyState===WebSocket.OPEN,watchdogNudges:conn.messages.filter(e=>e.type==='session.instructions.append').length,toolResults:conn.toolOutputs.length}));
            assert.notEqual(socket.readyState,WebSocket.OPEN,'Longer filler transcript disarmed watchdog without an answer or audio');
        }finally{socket.close();await sleep(300);}
    });
